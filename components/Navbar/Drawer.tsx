...
export default function Drawer({ children, isOpen, setIsOpen } : {children:React.ReactNode, isOpen : boolean, setIsOpen:React.Dispatch<React.SetStateAction<any>> }) {
  return (
    <main
      className={
        "md:hidden fixed overflow-hidden z-20 bg-gray-900 bg-opacity-25 inset-0 transform ease-in-out " +
        (isOpen
          ? " transition-opacity opacity-100 duration-500 translate-x-0  "
          : "hidden transition-all delay-500 opacity-0 translate-x-full  ")
      }
    >
      <section
        className={
